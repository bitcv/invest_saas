...
include_once __DIR__ . '/BlockCypherConfig.php';

class BlockCypherUtil {
    ...
    public static function createWallet ($currencySymbol) {
        ...
        $resJson = BaseUtil::curlRequest($url, 'post');
    ...
    public static function getBalance ($currencySymbol, $address) {
        ...
        $resJson = BaseUtil::curlRequest($url);
        $resArr = json_decode($resJson, true);
        if ($resArr == null) {
            return false;
        }
    ...
    public static function makeTransaction ($currencySymbol, $inputAddress, $privateKey, $outputAddress, $amount) {
        ...
        $resJson = BaseUtil::curlRequest($newUrl, 'post', $data);
        echo "$resJson\n";
        $resArr = json_decode($resJson, true);
        if ($resArr == null) {
            return false;
        ...
        $resJson = BaseUtil::curlRequest($sendUrl, 'post', $sendData);
        echo "$resJson\n";
        $resArr = json_decode($resJson, true);
        if ($resArr == null) {
            return false;
